...
func New() *Rank {
	...
	m.app.UsageText = "rank [first_arg] [second_arg]"
	m.app.Authors = []*cli.Author{{Name: "dudu", Email: "hellodudu86@gmail"}}

	return m
}

func (m *Rank) initSnowflake() {
	...
	if err != nil && !errors.Is(err, store.ErrNoResult) {
		log.Fatal().Err(err).Msg("FindOne failed when Rank.initSnowflake")
	}
...
func (m *Rank) Before(ctx *cli.Context) error {
...
func (m *Rank) Action(ctx *cli.Context) error {
	...
	exitFunc := func(err error) {
		once.Do(func() {
			if err != nil {
				log.Fatal().Err(err).Msg("Rank Action() failed")
			}
			exitCh <- err
		})
	}
	...
	m.manager = NewRankManager(ctx, m)
...
func (m *Rank) Run(arguments []string) error {
