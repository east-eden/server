...
func NewMicroService(ctx *cli.Context, m *Rank) *MicroService {
	// cert
		...
		micro.Server(
			grpc_server.NewServer(
				...
				server.RegisterCheck(func(context.Context) error {
					_, err := s.srv.Server().Options().Registry.GetService("rank")
	...
	// set environment
	os.Setenv("MICRO_SERVER_ID", ctx.String("rank_id"))
